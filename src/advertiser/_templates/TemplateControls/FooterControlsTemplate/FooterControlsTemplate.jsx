...
import ModalFooterConfigControls from "./components/ModalFooterConfigControls"

function FooterControlsTemplate() {
    const [isRetracted, setIsRetracted] = useState(true)
    const [isModalFooterOpen, setIsModalFooterOpen] = useState(false)

    if (isModalFooterOpen) {

        return (
            <ModalFooterConfigControls
        ...
        <div className={`w-full flex flex-col bg-white/80 backdrop-blur-sm shadow-lg p-4 
            ...
            <div className={`flex items-center ${isRetracted ? 'gap-12' : 'gap-2 w-full justify-between'}`}>
                <div className="flex items-center gap-2">
                    <DragHandle
                        ...
                        sx={{ fontSize: '1.5rem' }}
                    />
                    <span className="font-medium text-gray-700 tracking-wide text-sm">FOOTER</span>
                </div>
            </div>
            ...
            {!isRetracted && (
                <div className="mt-6 space-y-6">
                    <div
                        onClick={() => setIsModalFooterOpen(true)}
...
export default FooterControlsTemplate;